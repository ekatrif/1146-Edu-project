...
function makeOrder() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $(".empty p").html(
        `Ваш заказ принят. Номер для отслеживания - ${data.id}.`
      );
      ajaxFlag = false;
    },
...
function writeTable() {
  ...
  if (!tovardata.length) {
    $(".table, .form").remove();
    $("main").append(
      '<section class="empty"><div class="container"><p>Ваша корзина пуста!</p></div></section>'
    );
    return;
  }
  let tab = $(".table table");
  ...
  tab.append(
    `<tr><th colspan="2"></th><th colspan="2">Итого:</th><th colspan="2" class="itog">${sum}</th></tr>`
  );
}
function lightbox(aim) {
  ...
  if (w > sides * h) {
    // если ширина больше, чем нужна по соотношению сторон, уменьшаем ширину
    w = sides * h;
  } else if (w < sides * h) {
    // если высота больше, чем нужна по соотношению сторон, уменьшаем высоту
    h = w / sides;
  }
  ...
  let hlpstr =
    '<div class="screen"><div class="lightbox" style="margin-left:-' +
    leftfix +
    'px;"><button type="button">+</button><img src="' +
    src +
    '" style="width:' +
    w +
    "px;height:" +
  ...
  $(".screen").click(function (e) {
    if (
      e.target == document.querySelector(".screen") ||
      e.target == document.querySelector(".screen button")
    ) {
...
function galSlide(direction) {
  ...
  if (direction == "left") {
    // вычисляем новое положение с учетом направления движения
    hlpstr -= galstep;
    hlpstr -= galgap;
  } else {
    hlpstr += galstep;
  ...
  $(".gallery_rail").animate(
    {
      // плавно перемещаем блок
      left: hlpstr,
    },
    function () {
      // затем проверяем, должны ли работать кнопки в новом положении
      // такую же проверку можно поставить в $(function(){}), если у нас может быть на странице разное число картинок.
      if (
        $(".gallery_window").width() - $(".gallery_rail").width() >=
        parseInt(getComputedStyle($(".gallery_rail")[0]).left)
      ) {
...
function sliderRun(slideclass, direction) {
  ...
  let hlp = $("." + slideclass + "_block").index(
    $("." + slideclass + "_block.curr")
  );
  let width = $("." + slideclass + "_block.curr").width();
  let next;
  if (direction == "toleft") {
    next = hlp + 1;
    if (next > $("." + slideclass + "_block").length - 1)
      next -= $("." + slideclass + "_block").length;
    $("." + slideclass + "_block")
      .eq(next)
      .css("left", width + "px")
      .addClass("curr");
    $("." + slideclass + " .points span")
      .eq(next)
      .addClass("active");
    next = "-=" + width;
  ...
  $("." + slideclass + " .points span")
    .eq(hlp)
    .removeClass("active");
  $("." + slideclass + "_block.curr").animate(
    { left: next },
    2000,
    function () {
      $("." + slideclass + "_block")
...
function retimer() {
  ...
  $(".retaimer .num")[1].innerHTML = `${addChar(
    hour
  )}<span class="subnum">${multiple(hour, ["час", "часа", "часов"])}</span>`;
  delta = Math.floor(delta / 24);
  $(".retaimer .num")[0].innerHTML = `${delta}<span class="subnum">${multiple(
    delta,
    ["день", "дня", "дней"]
  )}</span>`;
}
